...
It is a common practice to publish snapshots and releases to different Maven repositories. A simple way to accomplish this is to configure the repository URL based on the project version.
...
Similarly, you can use a <<build_environment, project or system property>> to decide for one repository or another. The following example uses the release repository if the project property `release` is set, e.g. by running `gradle -Prelease publish`.
